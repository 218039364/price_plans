...
app.get("/api/phonebill", async (req, res) => {
  // write a query to return all the price plans
  const plans=await db.all(`SELECT sms_price,call_price FROM price_plan`);
...
app.post("/api/phonebill", async (req, res) => {
  ...
  const price_plan = await db.get(
    `SELECT id, plan_name, sms_price, call_price
    FROM price_plan WHERE plan_name = ?`,
    price_plan_name
  );
...
app.post("/api/price_plan/create", async (req, res) => {
  ...
  if (sql) {
    ...
    await db.run(
      ...
      [req.body.plan_name, req.body.sms_price, req.body.call_price]
    );
    res.json({
...
app.post("/api/price_plan/delete", async (req, res) => {
  ...
  if (found) {
    await db.run(
      "DELETE FROM price_plan WHERE plan_name=?",
      req.body.plan_name
    );
